...
public interface CommandeRepo extends CrudRepository<CommandeEntity, Long> {
    ...
    @Query(value = "SELECT SUM(cmd.qte*pr.pu) FROM products AS pr INNER JOIN commande AS cmd ON pr.id_products=cmd.product_id AND cmd.commande_id=1 AND cmd.date_commande BETWEEN :dateD AND :dateF ", nativeQuery = true)
    Double sumCommandeEnmodeInitial(@Param("dateD") String dateD, @Param("dateF") String dateF);

    @Query(value = "SELECT SUM(cmd.qte*pr.pu) FROM products AS pr INNER JOIN commande AS cmd ON pr.id_products=cmd.product_id AND cmd.commande_id=2 AND cmd.date_livraison BETWEEN :dateD AND :dateF ", nativeQuery = true)
    Double sumCommandeEnmodeLivraison(@Param("dateD") String dateD, @Param("dateF") String dateF);

    @Query(value = "SELECT SUM(cmd.qte*pr.pu) FROM products AS pr INNER JOIN commande AS cmd ON pr.id_products=cmd.product_id AND cmd.commande_id=4 AND cmd.date_retour BETWEEN :dateD AND :dateF ", nativeQuery = true)
