package com.herbal.ws.shared.excelExport;
    ...
    protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        for(CommandeResponse cmdi : list) {
         ...
         // GET ALL DATA TO CONTROLES
            ...
            row.createCell(2).setCellValue(cmdi.getFullNameClient());
            row.createCell(3).setCellValue(cmdi.getNumber());
            row.createCell(4).setCellValue(cmdi.getCity());
            row.createCell(5).setCellValue(cmdi.getProduct().getPu()+" MAD");
            row.createCell(6).setCellValue(cmdi.getQte());
            row.createCell(7).setCellValue(utils.ConvertDateToStr(cmdi.getDateCommande()));
            row.createCell(8).setCellValue(cmdi.getProduct().getPu()*cmdi.getQte() + "MAD");
        };
}
}